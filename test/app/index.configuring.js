...
test(`Add 'react' to package.json dependencies`, t => {
  ...
  t.is(context.mergeJson['package.json'].dependencies.react, '^15.4.2');
  ...
  t.is(context.mergeJson['package.json'].devDependencies['@types/react'], '^15.0.11');
  t.is(context.mergeJson['package.json'].devDependencies['@types/react-addons-test-utils'], '^0.14.17');
});

test(`Add 'router' to package.json dependencies`, t => {
  context.props = {router: 'router', modules: 'webpack'};
  TestUtils.call(context, 'configuring.pkg');
  t.is(context.mergeJson['package.json'].dependencies['react-router'], '^3.0.2');
  t.is(context.mergeJson['package.json'].devDependencies['@types/react-router'], '^3.0.3');
});

test(`Add 'router' cdn url to package.json dependencies`, t => {
  context.props = {router: 'router', modules: 'inject'};
  TestUtils.call(context, 'configuring.pkg');
  t.is(context.mergeJson['package.json'].dependencies['react-router'], 'https://cdnjs.cloudflare.com/ajax/libs/react-router/3.0.2/ReactRouter.min.js');
  t.is(context.mergeJson['package.json'].devDependencies['@types/react-router'], '^3.0.3');
});
