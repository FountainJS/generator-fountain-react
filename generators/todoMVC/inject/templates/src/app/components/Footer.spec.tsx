function footerSetup(propOverrides: any) {
  const props = assign({
    completedCount: 0,
    activeCount: 0,
    filter: SHOW_ALL,
...
describe('components', () => {
  describe('Footer', () => {
    ...
    it('should call onShow when a filter is clicked', () => {
      const {output, props} = footerSetup({});
      const [, filters] = output.props.children;
    ...
    it('shouldnt show clear button when no completed todos', () => {
      const {output} = footerSetup({completedCount: 0});
      const [, , clear] = output.props.children;
      expect(clear).toBe(undefined);
    });

    it('should render clear button when completed todos', () => {
      const {output} = footerSetup({completedCount: 1});
      const [, , clear] = output.props.children;
    ...
    it('should call onClearCompleted on clear button click', () => {
      const {output, props} = footerSetup({completedCount: 1});
      const [, , clear] = output.props.children;
      clear.props.onClick({});
